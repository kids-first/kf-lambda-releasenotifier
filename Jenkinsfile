  ...
  stages{
    ...
    stage('GetOpsScripts') {
      steps {
        ...
        sh '''
        git clone git@github.com:kids-first/kf-lambda-releasenotifier-config.git
        '''
      }
    }
    stage('Test') {
     steps {
       ...
       sh '''
       kf-lambda-releasenotifier-config/aws-lambda/ci-scripts/test_stage/test.sh
       '''
    ...
    stage('Deploy Dev') {
      ...
      steps {
        ...
        sh '''
        kf-lambda-releasenotifier-config/aws-lambda/ci-scripts/deploy_stage/deploy.sh dev
        '''
    ...
    stage('Deploy QA') {
     ...
     steps {
       ...
       sh '''
       kf-lambda-releasenotifier-config/aws-lambda/ci-scripts/deploy_stage/deploy.sh qa
       '''
       slackSend (color: '#41aa58', message: ":white_check_mark: DEPLOYED TO QA: (${env.BUILD_URL})")
     }
    }
    stage("Promotion releasenotifier-lambda to PRD") {
      when {
             expression {
               return env.BRANCH_NAME == 'master';
             }
      ...
      steps {
             script {
                     env.DEPLOY_TO_PRD = input message: 'User input required',
                                     ...
                                     parameters: [choice(name: 'releasenotifier-lambda: Deploy to PRD Environment', choices: 'no\nyes', description: 'Choose "yes" if you want to deploy the PRD server')]
             }
     }
    }
    stage('Deploy PRD') {
     ...
     steps {
       ...
       sh '''
       kf-lambda-releasenotifier-config/aws-lambda/ci-scripts/deploy_stage/deploy.sh prd
       '''
       slackSend (color: '#41aa58', message: ":white_check_mark: DEPLOYED TO PRD: (${env.BUILD_URL})")
     }
    }
    stage("Rollback releasenotifier-lambda?") {
      when {
             expression {
               return env.BRANCH_NAME == 'master';
             }
